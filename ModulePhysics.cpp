...
PhysBody* ModulePhysics::CreateChain(int x, int y, int* points, int size, bool isStatic)
{
	b2BodyDef body;
	if (isStatic)
	{
...
update_status ModulePhysics::PostUpdate()
{
	...
	if(!debug || !App->scene_intro->canDebugMode)
		return UPDATE_CONTINUE;
...
void PhysBody::SetPosition(int x, int y, float32 angle)
{
	b2Vec2 pos = { PIXEL_TO_METERS(x),PIXEL_TO_METERS(y) };
	body->SetTransform(pos, DEGTORAD * angle);	
}
